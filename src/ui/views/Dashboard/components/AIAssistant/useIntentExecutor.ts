...
async function getBase(): Promise<string> {
  ...
  try {
    ...
    /* unit tests / non-extension context */
  }
  return base;
}
...
async function assertSufficientBalance(
  ...
  const isNative =
    !tokenAddr || tokenAddr.toLowerCase() === NATIVE.toLowerCase();
    ...
    async (intent: ParsedIntent) => {
      ...
      try {
        ...
        if (intent.action === 'swap') {
          ...
          if (!fromAddr)
            throw new Error(`Unknown token "${fromTicker}" on ${chain}.`);
          if (!toAddr)
            throw new Error(`Unknown token "${toTicker}" on ${chain}.`);
          ...
          setSwapQuote({
            ...
            rate: `1 ${fromTicker} ≈ ${rateNum.toLocaleString('en', {
              maximumFractionDigits: 4,
            })} ${toTicker}`,
            gasUnits: quote.estimatedGas
